...
static uint32_t __attribute__ ((noinline)) grev_7(uint32_t x) {
    uint32_t rd=0;
    uint32_t msk = 0x80808080;
    rd  = (x & (msk >> 0)) >> 7;
    rd |= (x & (msk >> 1)) >> 6;
    rd |= (x & (msk >> 2)) >> 5;
    rd |= (x & (msk >> 3)) >> 4;
...
static uint32_t __attribute__ ((noinline)) clmul (uint32_t x, uint32_t y) {
    uint64_t rd = 0;
    for(int i = 0; i < 32;  i+=4) {
        rd |= ~(((y>>(i+0))&0x1)-1) & (x<<(i+0)) ;
        rd |= ~(((y>>(i+1))&0x1)-1) & (x<<(i+1)) ;
        rd |= ~(((y>>(i+2))&0x1)-1) & (x<<(i+2)) ;
        rd |= ~(((y>>(i+3))&0x1)-1) & (x<<(i+3)) ;
    }
    return (uint32_t)rd;
}

static uint32_t __attribute__ ((noinline)) clmulh(uint32_t x, uint32_t y) {
    uint64_t rd = 0;
    for(int i = 0; i < 32;  i+=4) {
        rd |= ~(((y>>(i+0))&0x1)-1) & ((uint64_t)x<<(i+0)) ;
        rd |= ~(((y>>(i+1))&0x1)-1) & ((uint64_t)x<<(i+1)) ;
        rd |= ~(((y>>(i+2))&0x1)-1) & ((uint64_t)x<<(i+2)) ;
        rd |= ~(((y>>(i+3))&0x1)-1) & ((uint64_t)x<<(i+3)) ;
    }
    return rd >> 32;
}

static uint64_t __attribute__ ((noinline)) clmuld(uint32_t x, uint32_t y) {
    uint64_t rd = 0;
    for(int i = 0; i < 32;  i+=4) {
        rd |= ~(((y>>(i+0))&0x1)-1) & ((uint64_t)x<<(i+0)) ;
        rd |= ~(((y>>(i+1))&0x1)-1) & ((uint64_t)x<<(i+1)) ;
...
// hi||lo = x*y
#define WIDE_CLMUL(hi,lo,x,y) {     \
    uint64_t tmp = clmuld(x,y);     \
    hi = tmp >> 32;                 \
    lo = tmp;                       \
}
...
void ghash_mul(gf128_t * z, const gf128_t * x, const gf128_t * h)
{
    ...
    WIDE_CLMUL(z4,z3,x3,y); // z4||z3 = x3*y
    WIDE_CLMUL(t1,z2,x2,y); // t1||z2 = x2*y
	z3 = z3 ^ t1;
    WIDE_CLMUL(t1,z1,x1,y); //
	z2 = z2 ^ t1;
    WIDE_CLMUL(t1,z0,x0,y); //
	z1 = z1 ^ t1;

#ifdef NO_SHIFTRED
	//  Mul reduction: 1 x CLMULH, 1 x CLMUL, 2 x XOR
    WIDE_CLMUL(t1,t0,z4,0x87)
	z1 = z1 ^ t1;
	z0 = z0 ^ t0;
#else
	...
	for (i = 2; i >= 0; i--) {				//  towards less significant
        ...
        WIDE_CLMUL(t1,t0,x3,y);
		z4 = z3 ^ t1;
        WIDE_CLMUL(t1,t2,x2,y);
		z3 = z2 ^ t0 ^ t1;
        WIDE_CLMUL(t1,t0,x1,y);
		z2 = z1 ^ t1 ^ t2;
        WIDE_CLMUL(t1,t2,x0,y);
		z1 = z0 ^ t0 ^ t1;
